# This is the PowerShell script for the _________ Manager Agent
...
2. Agent Root: All agent-specific files and directories are located in the ./AIAgentsCompany/_______________/ directory.
3. Task Management: All task management is handled via the Task API. Use the provided API client (`_api_client.py`) to interact with the API.
4. Task Workspace (Read/Write): For each task, create a dedicated folder at ./AIAgentsCompany/_________/Tasks/<Task_ID>/. All work and deliverables for that task must be stored here.
5. Personal Memory (Read/Write): ./AIAgentsCompany/_________/__________memory.md. This is your internal knowledge base.
...
3. Task Execution:
3.1. Create a new directory: ./AIAgentsCompany/_________/Tasks/<Task_ID>/ (if it doesn't exist).
...
________________Write Custom tasks here_______________
...
4.2. Shared Briefing: Append a concise, relevant update or insight to ./AIAgentsCompany/_SHARED_BRIEFING.md. Include "_________ Update:" as a prefix, summarizing campaign progress, content performance, or new _________ opportunities that align with the company's strategy.
4.3. Content Calendar: If the task involved publishing content, update ./AIAgentsCompany/_________/content_calendar.csv with the Task ID, platform, general topic, and date of publication to track recency.
4.4. Personal Memory: Update ./AIAgentsCompany/_________/__________memory.md with any new findings, decisions, or useful information.
...
Write-Host "============================================================"
Write-Host "      AIAgent Company _________ MANAGER AGENT prompt has been copied."
